...
(deftest test-read-and-repeat-results
  (new-parameters {:problem "Testing"
                   :name "test-runs"
                   :control (pr-str {:Bar [7 11] :Foo [2 3 5]})
  ...
  (let [runid (run-with-new-record
    ...
    (is (= (sort-by :simulation
                    (map #(update-in % [:params] read-string)
                         (get-results runid :control nil)))
           (sort-by :simulation
                    (map #(update-in % [:params] read-string)
    ...
    (is (= (sort-by :result (map #(update-in % [:params] read-string)
                                 (get-results runid :control [:result :Foo])))
           (sort-by :result
                    (map #(update-in % [:params] read-string)
